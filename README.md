...
完整列表：[`SKILLS.md`](SKILLS.md)。Angular 寫碼／審查仍會自己出現（`architect-first-gate`、`angular-dev-core-rules`、`zyxel-i18n-write`、`angular-code-review`）。`zyxel-rm-analyze` 亦然（`讀 RM` / `RM-#####`）。`zyxel-automation` 亦然（GitLab / Outline / Teams / Grafana / to-be-pulled）。
