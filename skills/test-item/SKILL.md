...
讀 [`CONTEXT.md`](CONTEXT.md)：確認文件種類是 **測試項目**，並套用 **去實作化**、**畫面用詞**、**清單寫完整**、**畫面動詞**、**畫面名不是所有格**。事實只來自指定分支的畫面改動。
...
套用 CONTEXT.md **去實作化**。每條草稿問：使用者在哪個畫面、做了什麼、看到什麼？寫不出來的刪掉。
...
**合格**（一句畫面摘要；目的條列；流程按獨立路徑分組；小標 = 目的原文）：

```
## 選擇 Organization 後不顯示 Limited-Time CNP/CNP+ Promotion，Devices 分頁的表格不顯示 Claim

### 使用者操作目的
- 選擇 Organization 後，畫面不顯示 Limited-Time CNP/CNP+ Promotion
...
```
- [ ] 已讀 CONTEXT.md；標題與兩區塊通過 去實作化、畫面用詞、清單寫完整、畫面動詞、畫面名不是所有格
